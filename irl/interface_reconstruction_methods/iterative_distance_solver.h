...
#ifndef IRL_INTERFACE_RECONSTRUCTION_METHODS_ITERATIVE_DISTANCE_SOLVER_H_
#define IRL_INTERFACE_RECONSTRUCTION_METHODS_ITERATIVE_DISTANCE_SOLVER_H_
...
template <class CuttingMethod, class CellType,
          UnsignedIndex_t kMaxPlanes =
...
public:
  ...
  /// reconstruction.
  const SmallVector<double, kMaxPlanes> &getDistances(void) const;

  /// \brief Return the distance for plane `a_p`.
  double getDistances(const UnsignedIndex_t a_p) const;
...
private:
  ...
  std::array<double, 2> bound_value_m;
  /// \brief The current solution
  double current_guess_m;
  /// \brief Characteristic length for scaling
  double characteristic_length_m;
  /// \brief Flag for flipped solution (always solve for VOF < 0.5 side)
  bool flipped_solution_m;
};

} // namespace IRL

#include "irl/interface_reconstruction_methods/iterative_distance_solver.tpp"
