...
void ProgressiveDistanceSolverParaboloid<CellType>::solveForDistance(
  ...
  sorted_distances_m.resize(number_of_vertices);
  UnsignedIndex_t count = 0;
  for (UnsignedIndex_t v = 0; v < number_of_vertices; ++v) {
    sorted_distances_m[count++] =
        signedDistance(half_edge_polytope.getVertex(v)->getLocation().getPt(),
                       aligned_paraboloid);
  }
  ...
  bounding_values[0] = std::max(bounding_values[0], sorted_distances_m.front());
  bounding_values[2] = std::min(bounding_values[2], sorted_distances_m.back());
