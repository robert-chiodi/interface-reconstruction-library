...
#ifndef IRL_INTERFACE_RECONSTRUCTION_METHODS_R2P_OPTIMIZATION_H_
#define IRL_INTERFACE_RECONSTRUCTION_METHODS_R2P_OPTIMIZATION_H_
...
#include "irl/optimization/levenberg_marquardt_scaled.h"
...
template <class CellType> class R2P_3D2P;
...
template <class CellType, UnsignedIndex_t kColumns> class R2PCommon {
  template <class R2PTypeForDebug> friend class R2PDebug;
...
public:
  ...
  PlanarSeparator
  runOptimization(R2PType *a_ptr_to_R2P_object,
  ...
  /// reference frame.
  void
  fillGeometryAndWeightVectors(const R2PNeighborhood<CellType> &a_neighborhood,
                               const double distance_multiplier,
                               const double volume_weight_switch);
...
private:
  ...
  UnitQuaternion
  getDeltaRotationQuat(const ReferenceFrame &a_reference_frame,
                       const Eigen::Matrix<double, columns_m, 1> &a_delta);

  /// \brief Return a reconstruction from R2P parameters
  PlanarSeparator
  getReconstructionFromR2PParam(const ReferenceFrame &a_reference_frame);
...
private:
  ...
  PlanarSeparator
  getReconstructionFromR2PParam(const ReferenceFrame &a_reference_frame,
                                const double a_beta, const double *a_distances);
...
template <class R2PType> class R2PDebug : public R2PType {
public:
  /// \brief Default constructor
  ...
  PlanarSeparator
  solve(const R2PNeighborhood<typename R2PType::cell_type> &a_neighborhood,
        const PlanarSeparator &a_reconstruction);
...
#include "irl/interface_reconstruction_methods/r2p_optimization.tpp"
