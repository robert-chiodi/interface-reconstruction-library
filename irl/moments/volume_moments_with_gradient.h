...
#ifndef IRL_MOMENTS_VOLUME_MOMENTS_WITH_GRADIENT_H_
#define IRL_MOMENTS_VOLUME_MOMENTS_WITH_GRADIENT_H_
...
class VolumeMomentsWithGradient {
 public:
  using gradient_type = GradientType;
  using moment_type = VolumeMoments;

  /// \brief Default constructor.
  VolumeMomentsWithGradient(void);

  /// \brief Constructor that initializes volume to value,
  /// want to allow implicit casting of double to Volume here.
  constexpr VolumeMomentsWithGradient(const double a_value);

  constexpr VolumeMomentsWithGradient(
      const double a_volume, const GradientType& a_volume_gradient,
      const PtWithGradient<GradientType>& a_centroid);

  static VolumeMomentsWithGradient fromScalarConstant(const double a_value);
  ...
  static VolumeMomentsWithGradient calculateMoments(GeometryType* a_geometry);
  ...
  /// \brief Return value of stored volume.
  PtWithGradient<GradientType>& centroid(void);

  /// \brief Return const reference to stored volume.
  const PtWithGradient<GradientType>& centroid(void) const;

  /// \brief Return value of stored gradient.
  GradientType& volume_gradient(void);

  /// \brief Return const reference to stored gradient.
  const GradientType& volume_gradient(void) const;

  /// \brief Overload += operator to update volume.
  VolumeMomentsWithGradient& operator+=(const VolumeMomentsWithGradient& a_rhs);

  /// \brief Overload *= operator to multiply by constant double
  VolumeMomentsWithGradient& operator*=(const double a_rhs);

  /// \brief Overload /= operator to divide by constant double
  VolumeMomentsWithGradient& operator/=(const double a_rhs);

  /// \brief Default destructor.
  ~VolumeMomentsWithGradient(void) = default;

 private:
  Volume volume_m;
  GradientType volume_gradient_m;
  PtWithGradient<GradientType> centroid_m;
};
...
#include "irl/moments/volume_moments_with_gradient.tpp"

#endif  // IRL_MOMENTS_VOLUME_MOMENTS_WITH_GRADIENT_H_ */
