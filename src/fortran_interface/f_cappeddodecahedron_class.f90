...
module f_CapDod_class
  ...
  interface calculateVolume
    module procedure CapDod_class_calculateVolume
  end interface 

  interface calculateCentroid
    module procedure CapDod_class_calculateCentroid
  end interface 
  ...
  interface
    ...
    function F_CapDod_calculateVolume(this) result(a_volume) &
      bind(C, name="c_CapDod_calculateVolume")
      import
      implicit none
      type(c_CapDod) :: this
      real(C_DOUBLE) :: a_volume
    end function F_CapDod_calculateVolume

    subroutine F_CapDod_calculateCentroid(this, a_centroid) &
      bind(C, name="c_CapDod_calculateCentroid")
      import
      implicit none
      type(c_CapDod) :: this
      real(C_DOUBLE), dimension(*), intent(out) :: a_centroid ! dimension(1:3)
    end subroutine F_CapDod_calculateCentroid
  ...
  contains
    ...
    function CapDod_class_calculateVolume(this) result(a_volume)
      implicit none
      type(CapDod_type), intent(inout) :: this
      real(IRL_double) :: a_volume
      a_volume = F_CapDod_calculateVolume(this%c_object)
    end function CapDod_class_calculateVolume

    function CapDod_class_calculateCentroid(this) result(a_centroid)
      implicit none
      type(CapDod_type), intent(in) :: this
      real(IRL_double), dimension(1:3) :: a_centroid
      call F_CapDod_calculateCentroid(this%c_object, a_centroid)
      return
    end function CapDod_class_calculateCentroid
